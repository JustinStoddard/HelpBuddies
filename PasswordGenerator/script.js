...
const generatePassword = (
  ...
  for (let x = 0; x < limit; x++) {
    const randomNum = generateRandomNum(actions.length);
    const actionNumber = parseInt(actions[randomNum]);
...
function writePassword() {
  ...
  if (wantedArray.length <= 1) {
    alert("Please select at leased two types of characters to generate your password with.");
    writePassword();
  } else {
